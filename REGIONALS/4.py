...
async def moveMotor(direction,side,degrees, speed=DEFAULT_SPEED):
    if (side == "top"):
        ...
        if direction == 'lift' and degrees <= 330 and abs_value < degrees:
            await motor.run_to_absolute_position(EXTENSION_MOTOR_TOP, degrees, speed, direction=motor.CLOCKWISE, stop = motor.HOLD)
        elif direction == 'drop' and abs_value > degrees:
            await motor.run_to_absolute_position(EXTENSION_MOTOR_TOP, degrees, speed, direction=motor.COUNTERCLOCKWISE, stop = motor.HOLD)
        else:
...
###############################################################################
    ...
    # submersible, sonar discovery and deliver unidentified creature
    await turnLeft(80)
    await drive(20)
    await turnLeft(30)
